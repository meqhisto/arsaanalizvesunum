...
class FiyatTahminModeli:
    ...
    def tahmin_yap(self, arsa_data: Dict) -> Dict:
        ...
        if bolge_stats and bolge_stats["analiz_sayisi"] > 0:
            ...
            # Model ağırlığı
            model_agirligi = 1 - (bolge_agirligi + benzer_agirligi)
            ...
            if benzer_arsalar:
                benzer_tahmini = sum(float(a["fiyat"]) for a in benzer_arsalar) / len(benzer_arsalar)
            else:
                benzer_tahmini = bolge_tahmini
            
            # Nihai tahmin hesaplama
            nihai_tahmin = (
                bolge_tahmini * bolge_agirligi +
                benzer_tahmini * benzer_agirligi +
                model_tahmini * model_agirligi
            )
            
            # Güven aralığı hesaplama
            std_hata = bolge_stats["std_birim_fiyat"] * arsa_data["metrekare"]
            guven_araligi = stats.norm.interval(0.95, nihai_tahmin, std_hata)
        else:
            nihai_tahmin = model_tahmini
            guven_araligi = (model_tahmini * 0.8, model_tahmini * 1.2)
    ...
    def bolge_trend_analizi(self, il: str, ilce: str, ay_sayisi: int = 12) -> Dict:
        ...
        if results:
            ...
            fiyatlar = [float(r.ort_birim_fiyat) for r in results]
